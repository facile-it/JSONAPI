...
public enum NoResourceObjectCheckError: Swift.Error {
...
public struct NoResourceObjectCheckErrors: Swift.Error {
    let problems: [NoResourceObjectCheckError]
}
...
public extension NoResourceObject {
    static func check(_ entity: NoResourceObject) throws {
        var problems = [NoResourceObjectCheckError]()
        ...
        guard problems.count == 0 else {
            throw NoResourceObjectCheckErrors(problems: problems)
        }
    }
}
